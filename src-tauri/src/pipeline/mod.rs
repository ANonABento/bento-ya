...
pub fn evaluate_exit_criteria(
    ...
    emit_pipeline(
        ...
        Some(format!("Exit type: {}", exit_type)),
    );
    ...
    if exit_met {
        emit_pipeline(
            ...
            &column.id,
            PipelineState::Evaluating,
            Some(format!("Exit criteria met: {}", exit_type)),
        );
    }
    ...
    let new_state = if exit_met {
        PipelineState::Idle
    } else if PipelineState::from_db_value(&task.pipeline_state) == PipelineState::Running {
...
pub fn mark_complete(
    ...
    match action {
        ...
        CompletionAction::Retry { attempt, max } => {
            ...
            log::info!(
                "[pipeline] Auto-retrying task {} (attempt {}/{})",
                task_id,
                attempt,
                max
            );
...
mod tests {
    ...
    fn test_pipeline_state_roundtrip() {
        for state in [
            ...
            assert_eq!(PipelineState::from_db_value(state.as_str()), state);
        }
    }

    #[test]
    fn test_pipeline_state_unknown_defaults_idle() {
        assert_eq!(PipelineState::from_db_value("garbage"), PipelineState::Idle);
        assert_eq!(PipelineState::from_db_value(""), PipelineState::Idle);
    }
}
