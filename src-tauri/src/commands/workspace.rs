...
pub fn seed_demo_data(state: State<AppState>, repo_path: String) -> Result<Workspace, AppError> {
    ...
    // ── Showcase tasks for the newer features ─────────────────────────────

    // Per-task model + priority (renders the opus + priority badges).
    db::insert_task_full(
        &conn,
        &db::NewTask {
            workspace_id: &ws.id,
            column_id: backlog_id,
            title: "Research vector index options",
            description: Some("Compare HNSW vs IVF for the embedding store"),
            model: Some("opus"),
            priority: Some("high"),
            ..Default::default()
        },
    ...
    // Dependency / DAG — this task is blocked until the spec task completes.
    let dep_parent = db::insert_task(
        &conn,
        &ws.id,
        backlog_id,
        "Spec the embedding schema",
        Some("Define columns + migration before implementation"),
    )?;
    db::insert_task_full(
        &conn,
        &db::NewTask {
            workspace_id: &ws.id,
            column_id: backlog_id,
            title: "Implement embedding store",
            description: Some("Blocked until the schema spec lands"),
            dependencies: Some(&format!("[\"{}\"]", dep_parent.id)),
            ..Default::default()
        },
    )?;

    // Interactive runtime mode — open this task's agent panel to drive the CLI
    // interactively (needs interactive mode enabled, set just below).
    db::insert_task_full(
        &conn,
        &db::NewTask {
            workspace_id: &ws.id,
            column_id: working_id,
            title: "Pair interactively on the failing test",
            description: Some("Runtime mode = interactive; drive claude from the agent panel"),
            model: Some("sonnet"),
    ...
    // MCP source attribution — an orchestrator task that "spawned" a child
    // (the child card shows the "spawned by …" badge).
    let orchestrator = db::insert_task(
        &conn,
        &ws.id,
        working_id,
        "Orchestrator: split the migration",
        Some("Breaks the work into subtasks via the MCP create_task tool"),
    )?;
    db::insert_task_full(
        &conn,
        &db::NewTask {
            workspace_id: &ws.id,
            column_id: working_id,
            title: "Write the down-migration",
            description: Some("Generated by the orchestrator agent"),
            created_by_task_id: Some(&orchestrator.id),
            recursion_depth: 1,
            ..Default::default()
        },
    )?;
    ...
    // Side effects (non-transactional, best-effort): turn on interactive mode
    // so the interactive task above is actually drivable, and drop some
    // markdown into the repo so the Files view has plans/notes to show.
    {
        let mut settings = crate::config::AppSettings::load();
        settings.interactive_mode_enabled = true;
        let _ = settings.save();
    }
    seed_demo_files(&repo_path);

    Ok(db::get_workspace(&conn, &ws.id)?)
}

/// Best-effort: write a few markdown files into the demo repo so the Files
/// viewer (which scans root `*.md`, `.context/**`, `.tickets/**`) has content.
/// Never clobbers existing files and never fails the seed.
#[cfg(debug_assertions)]
fn seed_demo_files(repo_path: &str) {
    let root = Path::new(repo_path);
    // Make the demo self-contained: create the repo dir if it doesn't exist yet
    // (seed_demo_data doesn't require a pre-existing git repo).
    let _ = std::fs::create_dir_all(root);

    let notes = root.join("DEMO-NOTES.md");
    if !notes.exists() {
        let _ = std::fs::write(
            &notes,
            "# Demo notes\n\nThis markdown is rendered by the **Files** view in the chef \
             and agent panels.\n\n- Plans, tickets, and notes live next to your chat and \
             terminal\n- Try the terminal search (Ctrl+Shift+F) and copy (Ctrl+Shift+C)\n\n```ts\n\
             // code blocks render too\nconst hello = 'world'\n```\n",
        );
    }

    let context_dir = root.join(".context");
    if std::fs::create_dir_all(&context_dir).is_ok() {
        let f = context_dir.join("architecture.md");
        if !f.exists() {
            let _ = std::fs::write(
                &f,
                "# Architecture\n\n> Demo context file.\n\nThe board columns are pipeline \
                 stages with trigger-driven automation. Agents run in per-task tmux sessions; \
                 the orchestrator coordinates at the workspace level.\n\n## Subsystems\n\n1. \
                 Pipeline / triggers\n2. Unified chat\n3. Per-task git worktrees\n",
            );
        }
    }

    let tickets_dir = root.join(".tickets");
    if std::fs::create_dir_all(&tickets_dir).is_ok() {
        let f = tickets_dir.join("sample-plan.md");
        if !f.exists() {
            let _ = std::fs::write(
                &f,
                "# Sample plan\n\n## Goal\n\nShip the embedding store.\n\n## Steps\n\n- [ ] \
                 Spec the schema\n- [ ] Implement the store\n- [ ] Backfill + index\n\n## Notes\n\n\
                 This is the kind of plan an agent reads from `.task.md` or that you keep in \
