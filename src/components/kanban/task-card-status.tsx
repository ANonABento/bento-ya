...
export function AttentionBanner({ attention }: { attention: AttentionItem }) {
  return (
    <div role="status" className="flex items-center gap-1.5 rounded bg-attention/10 px-2 py-1 text-xs text-attention">
...
export function QualityGateBanner({ reviewStatus }: { reviewStatus: ReviewStatus | null }) {
  ...
  return (
    <div role="status" className={`flex items-center gap-1.5 rounded px-2 py-1 text-xs ${colorClass}`}>
      {status === 'approved' ? (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="h-3 w-3 shrink-0" aria-hidden="true">
...
export function PipelineErrorBanner({
  ...
  return (
    <div role="alert" className="rounded bg-error/10 text-xs text-error">
