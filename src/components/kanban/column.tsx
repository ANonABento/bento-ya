...
const EMPTY_BATCH_QUEUE_STATE: BatchQueueLocalState = {
  isQueuing: false,
  total: 0,
  completed: 0,
  queuedTaskIds: [],
...
export const Column = memo(function Column({
  ...
  // Batch queue state
  const [batchQueueState, setBatchQueueState] =
    useState<BatchQueueLocalState>(EMPTY_BATCH_QUEUE_STATE)
  ...
  useEffect(() => {
    ...
    const completedCount = batchQueueState.queuedTaskIds.filter(
      (id) => taskById.get(id)?.agentStatus !== 'queued',
    ).length
    ...
    if (completedCount === batchQueueState.total) {
      setBatchQueueState(EMPTY_BATCH_QUEUE_STATE)
    }
  }, [
    taskById,
  ...
  const handleCancelQueue = useCallback(async () => {
    const remainingIds = batchQueueState.queuedTaskIds.filter(
      (id) => taskById.get(id)?.agentStatus === 'queued',
    )
    try {
    ...
    setBatchQueueState(EMPTY_BATCH_QUEUE_STATE)
  }, [batchQueueState.queuedTaskIds, taskById])
