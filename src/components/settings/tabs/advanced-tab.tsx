...
export function AdvancedTab() {
  ...
  const clearUpdateMessage = () => { setUpdateStatus(null) }
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        ...
        <p className="mb-3 text-xs text-text-secondary">Check GitHub releases for the latest version.</p>
          ...
          {updateStatus && (
            <div
              className={`rounded-md px-3 py-2 text-xs ${
                updateStatus.type === 'success' ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'
              }`}
            >
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    setUpdateStatus({ type: 'success', message: 'Update installed. Restart the app to complete the update.' })
                ...
                className="rounded-lg border border-accent bg-accent/10 px-3 py-2 text-sm text-text-primary transition-colors hover:bg-accent/20 disabled:cursor-not-allowed disabled:opacity-50"
              >
